...
void heapify(struct Item **a, int n, size_t i) {
    while (1) {
        ...
        if (l < n && a[i]->end > a[l]->end)
            i = l;
        if (r < n && a[i]->end > a[r]->end)
            i = r;
        if (i == j)
            break;
...
struct PriorityQueue *init_priority_queue(int n) {
    struct PriorityQueue *queue =
        (struct PriorityQueue *)calloc(1, sizeof(struct PriorityQueue));

    queue->count = 0;
    queue->capacity = n;
    queue->heap =
        (struct Item **)calloc(queue->capacity, sizeof(struct Item *));
...
bool empty(struct PriorityQueue *queue) { return queue->count == 0; }
...
void insert(struct PriorityQueue *queue, struct Item *item) {
    ...
    for (; i > 0 && queue->heap[(i - 1) / 2]->end > queue->heap[i]->end;
         i = (i - 1) / 2) {
...
void merge(int k, int l[], int *a[], int *result) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (l[item->key] > usage[item->key])
            insert(queue,
                   create_item(item->key, a[item->key][usage[item->key]++]));
